...
1. 创建布局树
   ...
   - 不可见的节点会被布局树忽略，比如`head`标签下面的全部内容和属性为`display: none;`节点
2. 布局计算
...
5. 三个和渲染流程有关的概念：重排、重绘、合成，如何减少重排重绘，优化 web 性能？

   - 重排：更新了元素的几何属性；重排需要更新完整的渲染流水线，开销最大
     ![](/Images/更新元素的几何属性-重排.png)
   - 重绘：更新元素的绘制属性；重绘省去了布局和分层阶段，执行效率要比重排高一些
     ![](/Images/更新元素背景-重绘.png)
   - 直接合成阶段：更改一个既不要布局也不要绘制的属性，渲染引擎将跳过布局和绘制，只执行后续的合成操作，大大提升了绘制效率
